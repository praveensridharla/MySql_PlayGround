...
**An interactive SQL learning environment for beginners.** Write SQL queries, insert data manually, import CSV files, and learn SQL with real sample databases — all running entirely in your browser using SQLite.

---

## 🚀 Quick Start (For Learning)

### Option 1: Open Directly in Browser (No Installation)
1. Extract the `mysql_playground.zip` file
2. Double-click `index.html` to open in your default browser
3. Start writing SQL queries!

### Option 2: Using a Local Server (Recommended)

**Why?** Some browsers block file:// URLs. Using a local server avoids CORS issues and is more reliable.
...
### Change Sample Data
Edit `data.js` to modify or add tables:

```javascript
const TABLES = {
  my_table: {
    label: 'My Table',
    icon: 'ti-package',  // Tabler Icon name
    cols: [
...
```html
<div class="pg-title">Your Custom Title</div>
<div class="pg-subtitle">Your subtitle</div>
```
...
```bash
scp -r mysql_playground/* user@yourserver.com:/var/www/html/sql
```
Access at: `https://yourserver.com/sql/index.html`
...
Build and run:
```bash
docker build -t mysql-playground .
docker run -p 8080:80 mysql-playground
```

Access at: `http://localhost:8080`

### Option 4: Containerized (Node.js)
Create `server.js`:
```javascript
...
Build and deploy on Heroku/Railway/Render.
...
2. Test thoroughly
3. Share improvements with friends
